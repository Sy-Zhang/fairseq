...
def consolidate_fsdp_shards(pth_prefix: str, save_prefix=None, strict=False, new_arch_name=None, no_stitch_megatron=False, megatron_part=None, is_ema=False) -> str:
    ...
    if megatron_part is not None:
        ...
        all_ckpt_files = [x for x in all_ckpt_files if f'model_part-{megatron_part}' in x]
    ...
    if 'decoder.embed_tokens.weight' in weights[0].keys():
        ...
        logger.info(f'This ckpt does not seem sharded. I see unflat params! like decoder.embed_tokens.weight shaped {shape}. Will just copy files and remove optim_state.')
        do_consolidate = False
    if do_consolidate:
        num_parts = find_num_parts(names)
        if num_parts:
            logger.info("consolidate_model_parallel")
            consolidated_weights = consolidate_model_parallel(metadata, names, strict, weights, parts=num_parts, no_stitch_megatron=no_stitch_megatron)
        else:
            logger.info("FSDP.consolidate_shard_weights")
            consolidated_weights = FSDP.consolidate_shard_weights(shard_weights=weights, shard_metadata=metadata,
                                                                  strict=strict)
    ...
    if dense:
        ...
        if no_stitch_megatron:
            ...
            for part_id, part_consolidated_weights in consolidated_weights.items():
                saved_paths.append(save_checkpoint(part_consolidated_weights, f"{save_prefix}-model_part-{part_id}"))
            return saved_paths
...
def consolidate_model_parallel(metadata, names, strict, weights, parts = 2, no_stitch_megatron=False):
    ...
    for k, v in model_parts.items():
        part_weights = FSDP.consolidate_shard_weights(shard_weights=v, shard_metadata=metadata_parts[k],
                                                      strict=strict)
...
def _handle_one(parts, is_weight):
    ...
    ret_val = torch.cat([p.unsqueeze(-1) for p in parts], dim=1).sum(1) - (init * (n_parts - 1))
    return ret_val
...
def handle_legacy_ln_(glued_model, n_parts):
    ...
    for i in range(n_layers):
        layer_weights = [glued_model.pop(f'decoder.layers.{i}.ffn_layernorm.lns.{p}.weight') for p in range(n_parts)]
        layer_biases = [glued_model.pop(f'decoder.layers.{i}.ffn_layernorm.lns.{p}.bias') for p in range(n_parts)]
        glued_model[f'decoder.layers.{i}.ffn_layernorm.weight'] = _handle_one(layer_weights, True)
        glued_model[f'decoder.layers.{i}.ffn_layernorm.bias'] = _handle_one(layer_biases, False)
...
def get_n_layers(glued_model):
    ...
    while True:
        if f'decoder.layers.{n_layers}.fc1.weight' in glued_model:
            ...
            assert n_layers > 0, f'found 0 layers bc no keys matching decoder.layers.0.fc1.weight'
            return n_layers
...
def glue_megatron_parts(model_parts):
    ...
    def assert_all_close(key):
        for part_id in range(len(model_parts)):
            if not torch.allclose(model_parts[part_id][key], model_parts[0][key]):
                err = (model_parts[part_id][key] - model_parts[0][key]).float().abs().max().item()
    ...
    for key in model_parts[0]:
        if 'qkv' in key:
            ...
            glued_model[key] = torch.cat([model_parts[part_id][key] for part_id in range(len(model_parts))])
            ...
            # Bias of CP gets concatenated
            if key.endswith('bias'):
                glued_bias = torch.cat([model_parts[part_id][key] for part_id in range(len(model_parts))])
                glued_model[key] = glued_bias
            # weights of CP gets concatenated along dim 0
            else:
                assert key.endswith('weight')
                glued_weight = torch.cat([model_parts[part_id][key] for part_id in range(len(model_parts))], dim=0)
        ...
        # FC2 is RP
            ...
            # weights of RP gets concatenated along dim 1
                ...
                assert key.endswith('weight')
                glued_weight = torch.cat([model_parts[part_id][key] for part_id in range(len(model_parts))], dim=1)
                glued_model[key] = glued_weight
